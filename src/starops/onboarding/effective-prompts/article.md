...
- ❌ 反例：`帮我看下系统最近怎么样`
- ✅ 正例：`@应用-checkout 看最近 1h 的核心业务指标（QPS / 成功率 / 延迟 PXX）是否偏离前 7 天基线`
...
涉及多步深入分析（如业务服务可靠性巡检的 5 Phase）时，建议保持在同一 thread 内串起来；新开 thread 后每个 Phase 都需要重新提供前序结果。
...
按需选择。简单场景（"看下 checkout 是否正常"）让 Agent 自由发挥即可；复杂场景（业务服务可靠性巡检、告警 RCA）建议人显式分 Phase，每个 Phase 输入输出明确，便于复盘和归档。
