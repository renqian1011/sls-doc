...
# 基于脚本进行 RDS 巡检 — SOP Skill

> **类型**：SOP Skill（与 `../practice.md` 一一对应、二者俱在）。教 Agent 怎么协助用户走完 5 步 SOP，最终落地一个活跃的 RDS 周期性巡检闭环。
>
> **不是** RDS 巡检本身的业务 Skill。业务 Skill 样品见 `../meta-skill-sample/SKILL.md`。
> 概念区分详见 [governance/policies/skill-package.md](../../../governance/policies/skill-package.md)。

## 能力边界

Agent **能做**：
- 读 / 拷贝本仓内的 meta skill 文件
- 校验 meta skill 包文件结构、Python 语法、`--list-cases` 输出
- 把 `../replay-prompt.md` 的 Prompt 主体抽出来交给用户（用户去 STAROps 智能运维助手粘贴）
...
> 对应 `../practice.md` §4 步骤 1。

### Agent 决策：选 Path A 还是 Path B

**默认走 Path A（直接复用本仓样品）**，除非用户明确说「要为我的自家场景演进 meta skill」或「现有样品不满足 X 需求」，才转 Path B。

### Path A：复用本仓 `../meta-skill-sample/`

Agent 执行：

```bash
cp -r ../meta-skill-sample/ <用户工作目录>/rds-inspection/
find <用户工作目录>/rds-inspection -type f | sort
```
...
### Path B：用 `../replay-prompt.md` 在 STAROps 智能运维助手现场生成

Agent 执行：

1. 读 `../replay-prompt.md` 「Prompt 主体」一节的代码块内容
...
> 对应 `../practice.md` §4 步骤 2。本步骤 Agent 不直接操作 STAROps；提供精确话术，由用户在控制台执行。
...
- 步骤 1 Path B 重试 2 次仍生成出 frontmatter 不合法的 SKILL.md（提示用户切回 Path A，并去 `../DESIGN.md` 「Replay Prompt 的演进」记录）
...
- 「我的 RDS 实例现在 CPU 高，帮我查一下」 → 路由到 `rds-inspection` meta skill（`../meta-skill-sample/`），做单次巡检即可，不需要走完整 SOP
- 「我要写一个新的 RDS 业务 Skill」 → 路由到 `../replay-prompt.md` + STAROps 智能运维助手
- 「帮我评审这个 RDS 巡检 SOP 写得好不好」 → 不属本 Skill；走 `governance/policies/review.md`

## 相关文件

- `../practice.md` — 人读版 SOP（本 Skill 的对偶文档，章节一一对应）
- `../replay-prompt.md` — 步骤 1 Path B 用到的 Prompt
- `../meta-skill-sample/SKILL.md` — Path A 直接复用的 meta skill 样品
- `../DESIGN.md` — SOP 设计决策与取舍（含 Replay Prompt 的演进记录入口）
- `../verification.md` — 真实跑通证据（人工补）
